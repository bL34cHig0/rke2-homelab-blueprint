...
4. `prometheus-grafana/` — create the storage class, install kube-prometheus-stack with the supplied values, then apply `prometheus-rules.yaml` and the Discord webhook secret.
5. `wordpress/` — apply manifests in the order documented in `wordpress/README.md` (MariaDB ConfigMap/Secret → ServiceAccount → PVC → Deployment → Service → WordPress equivalents → Certificate → Middleware → IngressRoute).
